...
module "observability" {
  ...
  monitored_account_ids              = [data.aws_caller_identity.current.id, var.gustavo_account_2]
...
module "promtail-lambda" {
  ...
  email_alert = "gustavosopeletoguimaraes@gmail.com"
  monitoring_accounts_id = [data.aws_caller_identity.current.id, var.gustavo_account_2]
